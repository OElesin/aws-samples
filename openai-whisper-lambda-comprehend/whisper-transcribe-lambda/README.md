...
```bash
whisper-transcribe-lambda$ sam build
```
...
```bash
whisper-transcribe-lambda$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
whisper-transcribe-lambda$ sam local start-api
whisper-transcribe-lambda$ curl http://localhost:3000/classify_digit
```
...
```bash
whisper-transcribe-lambda$ sam logs -n InferenceFunction --stack-name whisper-transcribe-lambda --tail
```
...
```bash
aws cloudformation delete-stack --stack-name whisper-transcribe-lambda
```
