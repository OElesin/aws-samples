...
```bash
whisper-insights-app$ sam build
```
...
```bash
whisper-insights-app$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
whisper-insights-app$ sam local start-api
whisper-insights-app$ curl http://localhost:3000/classify_digit
```
...
```bash
whisper-insights-app$ sam logs -n InferenceFunction --stack-name whisper-insights-app --tail
```
...
```bash
aws cloudformation delete-stack --stack-name whisper-insights-app
```
